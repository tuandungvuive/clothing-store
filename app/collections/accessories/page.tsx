...
const accessoriesSubcategories = [
  ...
  { id: "shoes", label: "Belts & Accessories" },
]
...
export default function AccessoriesCollectionPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered
  }, [accessoriesProducts, subcategories, colors, priceRange, inStock, sortBy])
  ...
  const hasActiveFilters =
    subcategories.length > 0 || colors.length > 0 || priceRange[0] > 0 || priceRange[1] < 500 || inStock
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {accessoriesSubcategories.map((cat) => (
            <div key={cat.id} className="flex items-center space-x-2">
              <Checkbox
                id={`accessories-cat-${cat.id}`}
                checked={subcategories.includes(cat.id)}
                onCheckedChange={(checked) => {
                  setSubcategories(checked ? [...subcategories, cat.id] : subcategories.filter((c) => c !== cat.id))
                }}
              />
              <Label htmlFor={`accessories-cat-${cat.id}`} className="text-sm font-normal cursor-pointer">
                {cat.label}
              </Label>
            </div>
          ))}
      ...
      <div className="flex items-center space-x-2">
        ...
        />
        <Label htmlFor="accessories-stock" className="text-sm font-normal cursor-pointer">
          In Stock Only
        </Label>
      </div>
    </div>
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 py-8">
          <CollectionHero
            ...
            productCount={accessoriesProducts.length}
          />
