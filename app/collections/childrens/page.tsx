...
const childrensSubcategories = [
  { id: "t-shirts", label: "Tops & T-Shirts" },
  { id: "shorts", label: "Bottoms" },
  { id: "jackets", label: "Outerwear" },
  { id: "shoes", label: "Shoes" },
]

const childrensSizes = ["2T", "3T", "4T", "5", "6", "7", "8", "10", "12", "14"]
...
export default function ChildrensCollectionPage() {
  ...
  const childrensProducts = products.filter((p) => p.category === "children")

  const filteredProducts = useMemo(() => {
    let filtered = [...childrensProducts]
    ...
    return filtered
  }, [childrensProducts, subcategories, sizes, colors, priceRange, inStock, sortBy])
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {childrensSubcategories.map((cat) => (
            <div key={cat.id} className="flex items-center space-x-2">
              <Checkbox
                id={`children-cat-${cat.id}`}
                checked={subcategories.includes(cat.id)}
                onCheckedChange={(checked) => {
                  setSubcategories(checked ? [...subcategories, cat.id] : subcategories.filter((c) => c !== cat.id))
                }}
              />
              <Label htmlFor={`children-cat-${cat.id}`} className="text-sm font-normal cursor-pointer">
                {cat.label}
              </Label>
            </div>
          ))}
      ...
      <div className="flex items-center space-x-2">
        <Checkbox id="children-stock" checked={inStock} onCheckedChange={(checked) => setInStock(checked as boolean)} />
        <Label htmlFor="children-stock" className="text-sm font-normal cursor-pointer">
          In Stock Only
        </Label>
      </div>
    </div>
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 py-8">
          <CollectionHero
            ...
            productCount={childrensProducts.length}
          />
